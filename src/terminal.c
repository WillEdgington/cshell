...
// terminal driver state is explicitly recovered if the program exits (abnormally or calls exit())
...
void cshell_terminal_mode_raw(void) {
  ...
  // VMIN: min number of bytes to read before read() returns. 1 makes read block until at least a single byte is ready
  // VTIME: timeout config in tenths of a second. 0 for infinite blocking (standard interactive loop)
  raw.c_cc[VMIN] = 1;
  raw.c_cc[VTIME] = 0;
...
void cshell_terminal_mode_canonical(void) {
  ...
  // TCSAFLUSH: tells terminal subsystem to wait for any unwritten output to finish rendering and discard any unread input before altering modes
